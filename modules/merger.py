...
def trusted_advisor_to_json():
    data = []
    with open('reports/AWS/aws_audit/final_report/trusted.json', 'r') as f:
        for line in f:
            k = json.loads(line)
            data.append(k['check'])
    data = set(data)
    ...
    for i in data:
        with open('reports/AWS/aws_audit/final_report/%s.txt', 'w+') as f:
            with open('reports/AWS/aws_audit/final_report/trusted.json', 'r') as j:
                for line in j:
                    k = json.loads(line)
                    if k['check'] == i:
                        f.write(line)
    final_json = {}
    report = []
    for f in glob.glob("reports/AWS/aws_audit/final_report/*.txt"):
        dict = {}
        data = []
        with open(f, 'r') as g:
            for line in g:
    ...
    with open('reports/AWS/aws_audit/final_report/final_json', 'w') as f:
...
def json_to_html_trusted():
    with open('./reports/AWS/aws_audit/final_report/trusted_advisor.html', 'w') as f:
        with open('./tools/prowler/template1.txt', 'r') as g:
             for line in g:
                  f.write(line)
        with open('./reports/AWS/aws_audit/final_report/final_json', 'r') as json_data:
...
def json_to_final_json():
    report = []
    for f in glob.glob("reports/AWS/aws_audit/delta/*.json"):
        dict = {}
        data = []
        with open(f, 'r') as g:
             for line in g:
    ...
    with open('reports/AWS/aws_audit/delta/final_json', 'w') as f:
         f.write(json.dumps(script_json))

    for i in script_json['report']:
        if i['check'] in ['CDN_AUDIT', 'CERT_AUDIT', 'DNS_AUDIT', 'ELB_AUDIT']:
            i['category'] = "network"
            with open('reports/AWS/aws_audit/delta/webnet.json', 'a+') as f:
            ...
            i['category'] = "database"
            with open('reports/AWS/aws_audit/delta/datastores.json', 'a+') as f:
                f.write(json.dumps(i))
                f.write('\n')
            ...
            with open('reports/AWS/aws_audit/delta/notification.json', 'a+') as f:
                f.write(json.dumps(i))
                f.write('\n')
            log.info("aws final report - notification", extra=i)
        else:
            i['category'] = "management"
            with open('reports/AWS/aws_audit/delta/configs.json', 'a+') as f:
                f.write(json.dumps(i))
                f.write('\n')
            log.info("aws final report - configs", extra=i)

def json_to_html_prowler():
    with open('./reports/AWS/aws_audit/delta/prowler_report.html', 'w') as f:
        with open('./tools/prowler/template1.txt', 'r') as g:
             for line in g:
                  f.write(line)
...
def merge_json():
    with open('reports/AWS/aws_audit/delta/final_json', 'r') as f:
        for line in f:
            j1 = json.loads(line)
...
def persistent_json(json_file):
    ...
    checks = set(checks)
    dict = {}
    with open('reports/AWS/aws_audit/delta/final_diff.json', 'w') as g:
        for check in checks:
            dict = {}
            data = []
            with open(json_file , 'r') as f:
...
def persis(j1,j2):
    f=open("./reports/AWS/aws_audit/delta/diff.json", "a+")
    ...
    for d1 in data1['report']:
        for d2 in data2['report']:
            if d2['check'] == d1['check']:
                persis(d1,d2)
    persistent_json("./reports/AWS/aws_audit/delta/diff.json")



def persistent_files():
    dirs = os.listdir("./reports/AWS/aws_audit/")
    if len(dirs) == 1:
        ...
        json_to_html('./reports/AWS/aws_audit/delta/final_diff.json',
                     './reports/AWS/aws_audit/delta/diff.html')


def merge():
    if os.stat('reports/AWS/aws_audit/final_report/trusted.json').st_size != 0:
        trusted_advisor_to_json()
    json_to_final_json()
    json_to_html_prowler()
    json_to_html('./reports/AWS/aws_audit/delta/webnet.json',
                 './reports/AWS/aws_audit/delta/webnet.html')
    json_to_html('./reports/AWS/aws_audit/delta/datastores.json',
                 './reports/AWS/aws_audit/delta/datastores.html')
    json_to_html('./reports/AWS/aws_audit/delta/notification.json',
                 './reports/AWS/aws_audit/delta/notification.html')
    json_to_html('./reports/AWS/aws_audit/delta/configs.json',
                 './reports/AWS/aws_audit/delta/configs.html')
    merge_json()
    persistent_files()
    subprocess.check_output(
        ['cp -R ./tools/template/* ./reports/AWS/aws_audit/final_report/'], shell=True)
    subprocess.check_output(['rm ./reports/AWS/aws_audit/final_report/report_azure.html'], shell=True)
    webbrowser.open('file://' + os.path.realpath("./reports/AWS/aws_audit/final_report/report.html"))
    fin = os.path.realpath("./reports/AWS/aws_audit/final_report/report.html")
